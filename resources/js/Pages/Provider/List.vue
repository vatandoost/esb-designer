<template>
  <Head :title="__('messages.providers')" />

  <App :breadcrumbs="[{ label: __('messages.providers'), to: route('provider.index') }]">
    <Link :href="route('provider.create')">
      <Button
    ...
    <DataTable :value="items" stripedRows responsiveLayout="scroll">
      ...
      <Column field="id" header="">
        <template #body="slotProps">
          <Link
            :href="route('provider.edit', { provider: slotProps.data.id })"
            class="mr-2"
          >
  ...
  setup(props) {
    ...
    function deleteDb(id) {
      confirm.require({
        ...
        accept: () => {
          Inertia.visit(route("provider.delete", { provider: id }), {
            method: "DELETE",
          });
        },
        reject: () => {
